...
# 定义 matrix_to_quaternion 函数 (从 overfit_train.py 复制过来，确保一致性)
...
def main():
    ...
    parser.add_argument('--proj_dir', type=str, default='/Disk_16TB/zhouhaowei/code/network_GAS/TMI/data/TCIA_projections_512res_600numproj/test/patient_00001_img_02_cone', help='包含投影和相机参数的测试数据目录路径')
    parser.add_argument('--config_path', type=str, default='configs/default_config.yaml', help='模型配置文件路径 (必须与训练时使用的配置完全一致)')
    parser.add_argument('--ckpt_path', type=str, default='/Disk_16TB/zhouhaowei/code/network_GAS/TMI/experiments_out/2025-04-25/10-24-50/logs/1/model_latest.pth', help='通用训练保存的模型权重路径 (例如 model_latest.pth 或 model_best.pth)')
    ...
    # 检查 ckpt_path 是否为默认值，如果是则提示用户修改
    if "<timestamp>" in args.ckpt_path or "<run_name>" in args.ckpt_path:
        logger.error("请提供有效的 --ckpt_path，替换掉 <timestamp> 和 <run_name>。")
        sys.exit(1)
    ...
    try:
        ...
        sys.exit(1)
    # --- 配置加载结束 ---
    '''{'defaults': [{'wandb': 'defaults'}, {'hydra': 'defaults'}, {'cam_embd': 'defaults'}, '_self_'], 'general': {'device': 0, 'seed': 0, 'mixed_precision': False}, 'data': {'data_path': '/Disk_10TB/zhouhaowei/data', 'training_resolution': 512, 'input_images': 1, 'batch_size': 5, 'num_workers': 0, 'input_views': 1}, 'opt': {'iterations': 5000000, 'mem_clean_interval': 1000, 'min_lr_factor': 0.1, 'base_lr': 1e-05, 'betas': [0.9, 0.999], 'loss': 'l1', 'use_fixed_lr': True, 'imgs_per_obj': 4, 'accumulation_steps': 1, 'ema': {'use': False, 'beta': 0.9999, 'update_every': 10, 'update_after_step': 100}, 'pretrained_ckpt': 'None', 'w_l12': 1.0, 'w_ssim': 0.2, 'w_tv': 0, 'lambda_xyz_boundary': 0.1, 'warmup_steps': 1000, 'weight_decay': 0.01}, 'model': {'max_sh_degree': 1, 'name': 'SingleUNet', 'scale_bias': 0.03, 'scale_scale': 0.1, 'xyz_scale': 0.1, 'xyz_bias': 0, 'xyz_range': 1, 'depth_scale': 1.0, 'depth_bias': 0, 'density_scale': 0.1, 'density_bias': -3, 'z_near': 6.5, 'z_far': 9.5, 'use_zrange': True, 'network_without_offset': False, 'network_with_offset': True, 'attention_resolutions': [16], 'num_blocks': 4, 'cross_view_attention': True, 'base_dim': 128, 'isotropic': False, 'num_gaussians': 1}, 'logging': {'wandb_project': 'general train_LIDC', 'wandb_run_name': '1', 'disable_wandb': False, 'wandb_offline': False, 'ckpt_iterations': 500, 'val_log': 100, 'loss_log': 10, 'render_log': 10, 'volume_save_interval': 10000, 'output_dir': '/Disk_16TB/zhouhaowei/code/network_GAS/TMI/experiments_out', 'resume_ckpt': 'None', 'ckpt_every': 100}, 'cam_embd': {'embedding': 'pose', 'encode_embedding': 'positional', 'dimension': 32, 'method': 'film'}}'''
    print(cfg)
    # 加载模型
